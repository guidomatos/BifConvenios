                                        ...
                                        <asp:DataGrid ID="dgProcesos" runat="server" Width="720px" AutoGenerateColumns="False"
                                            ...
                                            <Columns>
                                                <asp:TemplateColumn HeaderText="Nombre de la Empresa" ItemStyle-Width="230">
                                                    <ItemTemplate>
                                                        <asp:HyperLink runat="server" Visible='<%# ctype(DataBinder.Eval(Container.DataItem, "ShowDelete"), Boolean)%>'
                                                            Text='<%# DataBinder.Eval(Container, "DataItem.Nombre_Cliente") %>' NavigateUrl='<%# DataBinder.Eval(Container, "DataItem.Codigo_Proceso", "ResultadoProcesoCronogramaFuturo.aspx?id={0}")  %>'>
                                                        </asp:HyperLink>
                                                        <asp:Literal runat="server" ID="ltrl" Visible='<%# (Not ctype(DataBinder.Eval(Container.DataItem, "ShowDelete"), Boolean))%>'
                                                            Text='<%# DataBinder.Eval(Container, "DataItem.Nombre_Cliente") %>'>
                                                        </asp:Literal>
                                                    </ItemTemplate>
                                                </asp:TemplateColumn>
                                                <asp:TemplateColumn HeaderText="Periodo">
                                                    <ItemTemplate>
                                                        <asp:Label runat="server" Text='<%# BIFConvenios.Periodo.GetMonthByNumber ( DataBinder.Eval(Container, "DataItem.Mes_Periodo") ) %> '>
                                                        </asp:Label>
                                                ...
                                                <asp:TemplateColumn HeaderText="Reporte">
                                                    <ItemTemplate>
                                                        <a href="javascript:GenerarReporte('<%#DataBinder.Eval(Container.DataItem, "Codigo_Proceso") %>','<%#DataBinder.Eval(Container.DataItem, "Nombre_Cliente")%>','<%#DataBinder.Eval(Container.DataItem, "Anio_periodo")%>','<%#DataBinder.Eval(Container.DataItem, "Mes_Periodo")%>','<%#DataBinder.Eval(Container.DataItem, "Fecha_ProcesoAS400")%>')">
                                                            <img src='images/agenda.gif' border='0' alt='Generar Listado de Cuotas por Vencer'></a>
                                                    </ItemTemplate>
                                                </asp:TemplateColumn>
                                                <asp:TemplateColumn ItemStyle-Width="40">
                                                    <ItemTemplate>
                                                        <%#MostrarPaginaConsulta(DataBinder.Eval(Container.DataItem, "Codigo_Proceso"), DataBinder.Eval(Container.DataItem, "Codigo_Cliente"))%>                                                       
                                                    </ItemTemplate>
                                                </asp:TemplateColumn>
                                                <asp:TemplateColumn>
                                                    <ItemTemplate>
                                                        <%#MostrarElemento(ctype(DataBinder.Eval(Container.DataItem, "ShowDelete"), Boolean), DataBinder.Eval(Container.DataItem, "Codigo_Proceso"),  DataBinder.Eval(Container.DataItem, "Nombre_Cliente"), DataBinder.Eval(Container.DataItem, "Anio_periodo"), DataBinder.Eval(Container.DataItem, "Mes_Periodo"),  DataBinder.Eval(Container.DataItem, "Fecha_ProcesoAS400"))%>
                                                    </ItemTemplate>
                                                </asp:TemplateColumn>
                                            </Columns>
                                        </asp:DataGrid></div>
